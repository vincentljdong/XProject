...
public class CacheInterceptorOffline extends CacheInterceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        LogWraper.d("RxRetrofit", "request url :" +  request.url().url());
        LogWraper.d("RxRetrofit", "request tag :" +  request.tag().toString());
        LogWraper.d("RxRetrofit", "request header :" +  request.headers().toString());
        if (!NetworkUtil.isNetworkAvailable(context)) {
            LogWraper.e("RxRetrofit", " no network load cache:"+ request.cacheControl().toString());
